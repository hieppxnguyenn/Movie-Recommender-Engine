...
import json
from khdl2 import KNearestNeighbours
from bs4 import BeautifulSoup
import requests,io
import PIL.Image
from urllib.request import urlopen

with open('/Users/linh/PycharmProjects/pythonProjects/movie_data.json', 'r+', encoding='utf-8') as f:
    data = json.load(f)
with open('/Users/linh/PycharmProjects/pythonProjects/movie_titles.json', 'r+', encoding='utf-8') as f:
...
def run():
    img1 = Image.open('/Users/linh/Downloads/logo.png')
    img1 = img1.resize((250,250),)
    st.image(img1,use_column_width=False)
    st.title("Movie Recommender Engine")
    st.markdown('''<h4 style='text-align: left; color: #d73b5c;'>"IMDB 1000 Movies Dataset"</h4>''',
    ...
    if cat_op == category[0]:
        ...
        if dec == 'No':
            ...
            if select_movie == '--Select--':
                ...
                no_of_reco = st.slider('Number of movies you want to see:', min_value=5, max_value=20, step=1)
                genres = data[movies.index(select_movie)]
                test_points = genres
                table = KNN_Movie_Recommender(test_points, no_of_reco+1)
                table.pop(0)
                c = 0
                st.success('Some of the movies from our Recommendation, take a look')
                for movie, link, ratings in table:
                    c += 1
                    st.markdown(f"({c})[ {movie}]({link})")
        ...
        if dec == 'No':
            ...
            if sel_gen:
                ...
                for movie, link, ratings in table:
                    ...
                    st.markdown('IMDB Rating: ' + str(ratings) + '⭐')
run()
#streamlit run /Users/linh/PycharmProjects/pythonProjects/khdl3.py
